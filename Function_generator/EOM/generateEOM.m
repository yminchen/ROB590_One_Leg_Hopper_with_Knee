...
T = 0.5*( m1*sum(vCoG1.^2) + J1*vphi^2 ...
        + m2*sum(vCoG2.^2) + J2*(vphi+valpha)^2 ...
        + m3*sum(vCoG3.^2) + J3*(vphi+valpha+vbeta)^2);
...
matlabFunction(CoG2,'file','Functions\CoGThigh','vars',{q,param});
matlabFunction(Knee,'file','Functions\posKnee','vars',{q,param});
matlabFunction(CoG3,'file','Functions\CoGShank','vars',{q,param});
